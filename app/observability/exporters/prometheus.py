...
def render_prometheus_metrics(
    ...
    for model, version in sorted(loaded_models.items()):
        lines.append(f'serving_model_loaded{{model="{model}",version="{version}"}} 1')
    if model_processes:
        _append_worker_metrics(lines, model_processes)
...
def _append_worker_metrics(lines: list[str], model_processes: dict[str, dict]) -> None:
