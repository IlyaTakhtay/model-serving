...
def validate_inputs(
    ...
    if not isinstance(raw_inputs, list) or not raw_inputs:
        raise InvalidTensorError("Inference header must contain non-empty inputs list")
    ...
    for index, item in enumerate(raw_inputs):
        item = _require_object(item, "Input", index)
        name, datatype, shape = _parse_input_header(item, index)
        expected = expected_inputs.get(name)
        if expected is None:
            raise InvalidTensorError(f"Input '{name}' is not declared by loaded model")
        if datatype not in DATATYPE_TO_DTYPE:
            raise InvalidTensorError(f"Unsupported binary input datatype '{datatype}'")
        if datatype != expected.datatype:
            raise InvalidTensorError(
                f"Input '{name}' expects datatype {expected.datatype}, got {datatype}"
            )
        if not _shape_matches(expected.shape, shape):
            raise InvalidTensorError(
                f"Input '{name}' expects shape {expected.shape}, got {shape}"
            )

        binary_size = _binary_size(item, name, datatype, shape)
...
def validate_output_names(header: dict[str, Any], metadata: ModelMetadata) -> list[str]:
    ...
    for index, item in enumerate(raw_outputs):
        item = _require_object(item, "Output", index)
        if "name" not in item:
...
def make_binary_response_header(
    request_id: str | None,
    model_name: str,
    model_version: str,
...
def _parse_input_header(item: dict[str, Any], index: int) -> tuple[str, str, list[int]]:
    try:
        name = str(item["name"])
...
def _binary_size(
    item: dict[str, Any], name: str, datatype: str, shape: list[int]
) -> int:
...
def _shape_matches(expected: list[int], actual: list[int]) -> bool:
...
def _require_object(item: Any, label: str, index: int) -> dict[str, Any]:
